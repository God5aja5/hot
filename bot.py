...
def format_progress(job):
    ...
    if job.checker_type == "xbox":
        # Get retry statistics from the Xbox checker
        retry_stats = xbox_checker.stats
        lines = (
            ...
            f"Errors: {job.errors} | Fast Retries: {retry_stats.retries}\n"
...
def send_hits_file(chat_id, service_hits, hits_total, caption, reply_to_message_id=None, checker_type="inboxer"):
    if not service_hits:
        ...
        try:
            ...
            temp.close()
            print(f"[FILE] Sending no-hits file to {chat_id}")
            with open(temp.name, "rb") as f:
                bot.send_document(chat_id, f, caption=caption, reply_to_message_id=reply_to_message_id)
            print(f"[FILE] No-hits file sent successfully to {chat_id}")
        except Exception as e:
            print(f"[ERROR] Failed to send no-hits file to {chat_id}: {e}")
        finally:
    ...
    try:
        print(f"[FILE] Creating ZIP file for {chat_id} with {len(service_hits)} files")
            ...
            if checker_type == "xbox":
                ...
                for filename in xbox_files:
                    ...
                    if lines:
                        ...
                        print(f"[FILE] Added {filename} with {len(lines)} lines")
                    else:
                        # Create empty file
                        zf.writestr(filename, "")
                        print(f"[FILE] Created empty {filename}")
        ...
        print(f"[FILE] Sending ZIP file to {chat_id}")
        with open(temp.name, "rb") as f:
            bot.send_document(chat_id, f, caption=caption, reply_to_message_id=reply_to_message_id)
        print(f"[FILE] ZIP file sent successfully to {chat_id}")
    except Exception as e:
        print(f"[ERROR] Failed to send ZIP file to {chat_id}: {e}")
        import traceback
        traceback.print_exc()
    finally:
        if os.path.exists(temp.name):
...
def start_job(chat_id, user_id, combos, user_link, is_admin_user, reply_to_message_id, checker_type="inboxer"):
    with job_lock:
        if not is_admin_user and user_id in active_by_user:
            ...
            return
        # Use appropriate thread count based on checker type
        threads = XBOX_THREADS if checker_type == "xbox" else INBOXER_THREADS
        job = Job(user_id, chat_id, len(combos), threads, user_link, reply_to_message_id, checker_type)
        jobs[job.job_id] = job
        if not is_admin_user:
    ...
    runner.start()
    print(f"[START] job_id={job.job_id} user_id={user_id} combos={len(combos)} threads={threads} checker_type={checker_type}")
...
def handle_start(message):
    ...
    welcome = (
        ...
        f"• {INBOXER_THREADS} Threads for Inboxer checker\n"
        f"• {XBOX_THREADS} Threads for Xbox checker (optimized)\n"
        "• Results Sent As .zip\n\n"
...
def main():
    ...
    print(f"[BOOT] Inboxer threads: {INBOXER_THREADS}")
    print(f"[BOOT] Xbox threads: {XBOX_THREADS}")
